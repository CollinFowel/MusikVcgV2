...
async def stop(_, message: Message):
    ...
    if chat_id not in callsmusic.pytgcalls.active_calls:
        ...
        await message.reply_text("❌ Berhenti mendengarkan lagu!")
...
async def skip(_, message: Message):
    ...
    chat_id = chid
    if chat_id not in callsmusic.pytgcalls.active_calls:
        await message.reply_text("❗ Tidak bisa skip karena tidak ada lagu yang sedang diputar!")
    else:
    ...
    if not qeue:
        return
    await message.reply_text(f"- Melompati lagu **{skip[0]}**\n- Lagu yang diputar sekarang **{qeue[0][0]}**")


@Client.on_message(filters.command("channelreload"))
@errors
